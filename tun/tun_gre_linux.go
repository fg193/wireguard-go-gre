...
func CreateGRETun(nameA string, mtu int) (Device, error) {
	...
	for i := range ifaces {
		...
		if err := nlAddGRE(iface.name, iface.local, iface.remote); err != nil {
			cleanup()
...
func (t *GRETun) Close() error {
	...
	t.closeOnce.Do(func() {
		...
		if err := nlDelLink(t.name); err != nil && firstErr == nil {
			firstErr = err
		}
		if err := nlDelLink(t.name + greTunPeerSuffix); err != nil && firstErr == nil {
			firstErr = err
		}
...
// nlAddGRE creates a single GRE (L3) interface.
// Equivalent to: ip link add <name> type gre local <local> remote <remote>
func nlAddGRE(name string, local, remote netip.Addr) error {
	const (
		iflaInfoKind  = 1
		iflaInfoData  = 2
		iflaGreLocal  = 6
...
// nlDelLink removes an interface.
